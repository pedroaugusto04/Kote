...
import {
  injectHandoffIntoActiveTerminal,
  persistLastHandoffFile,
} from '../ai-history/handoff/session-handoff.manager';
      ...
      async (message) => {
        switch (message.command) {
          case 'acceptAndInject': {
            ...
            persistLastHandoffFile(this.currentMarkdown);
            const injected = injectHandoffIntoActiveTerminal(this.currentMarkdown);
            if (injected) {
              vscode.window.showInformationMessage(
                `Kote: Session handoff injected into active terminal for ${this.params.targetProviderName}.`,
              );
            } else {
              vscode.window.showInformationMessage(
                `Kote: Session handoff context copied to clipboard for ${this.params.targetProviderName}.`,
              );
            }
