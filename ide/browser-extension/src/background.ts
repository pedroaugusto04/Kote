...
// Proactively refresh access token on extension startup
async function initializeSession(apiUrl: string): Promise<void> {
  const config = await chrome.storage.local.get(['refreshToken', 'authMethod', 'accessToken', 'connectionToken']);
  
  // Handle email auth: refresh using refresh token
  if (config.authMethod === 'email' && config.refreshToken && !config.accessToken) {
    try {
      const newAccessToken = await refreshAccessToken(apiUrl);
      console.log('Email session refreshed on startup');
    } catch (error) {
      console.warn('Failed to refresh email session on startup:', error);
      // Clear invalid tokens
      await chrome.storage.local.remove(['accessToken', 'refreshToken', 'connectionToken', 'authMethod']);
    }
  }
  
  // Handle token auth: exchange connection token for access/refresh tokens
  if (config.authMethod === 'token' && config.connectionToken && !config.accessToken) {
    try {
      ...
      const exchangeRes = await fetch(`${apiUrl}/api/auth/exchange-connection-token`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ connectionToken: config.connectionToken }),
      });
      ...
      console.log('Token session refreshed on startup');
    } catch (error) {
      console.warn('Failed to refresh token session on startup:', error);
      // Clear invalid tokens
      await chrome.storage.local.remove(['accessToken', 'refreshToken', 'connectionToken', 'authMethod']);
    }
  }
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  let apiUrl = config.apiUrl || 'https://knowledgebase.sbs/kote';
  apiUrl = apiUrl.trim().replace(/\/$/, '');
  if (apiUrl.endsWith('/api')) {
    apiUrl = apiUrl.slice(0, -4);
  }
  await initializeSession(apiUrl);
});
...
async function saveClippedNote(clip: ClipPayload, tags: string[] = [], projectSlugOverride?: string): Promise<any> {
  ...
  if (authMethod === 'email' && storedAccessToken) {
    ...
    // Token auth: exchange connection token via HTTP API
    const headers: Record<string, string> = { 'Content-Type': 'application/json' };
    if (chrome.runtime.id) {
      headers['Origin'] = `chrome-extension://${chrome.runtime.id}`;
    }
    ...
    await chrome.storage.local.set({ accessToken, refreshToken: data.refreshToken });
  } else {
