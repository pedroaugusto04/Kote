...
function HelpAccordionItem({ item, index, isOpen, onToggle }: {
  item: HelpItem;
  index: number;
  isOpen: boolean;
  onToggle: () => void;
  ...
  return (
    <div className={`help-accordion-item ${isOpen ? 'open' : ''}`}>
      <button
        ...
        id={`help-item-trigger-${index}`}
        aria-controls={`help-item-content-${index}`}
      >
        <span className="help-accordion-index">{String(index + 1).padStart(2, '0')}</span>
        <span className="help-accordion-label">{item.title}</span>
        <span className={`help-accordion-chevron ${isOpen ? 'open' : ''}`}>
          <IconChevronDown />
        </span>
      </button>
      <div
        className={`help-accordion-content ${isOpen ? 'open' : ''}`}
        id={`help-item-content-${index}`}
        role="region"
        aria-labelledby={`help-item-trigger-${index}`}
      >
