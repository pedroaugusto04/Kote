...
export function readEnvironment(env = process.env): RuntimeEnvironment {
  ...
  return {
    ...
    aiSessionSynthesisProvider: (String(env.KB_AI_SESSION_SYNTHESIS_PROVIDER || defaultChatProvider).trim().toLowerCase() as RuntimeEnvironment['aiSessionSynthesisProvider']),
    aiSessionSynthesisBaseUrl: String(env.KB_AI_SESSION_SYNTHESIS_BASE_URL || defaultChatBaseUrl).trim(),
    aiSessionSynthesisModel: String(env.KB_AI_SESSION_SYNTHESIS_MODEL || defaultChatModel).trim(),
    aiSessionSynthesisApiKey: String(env.KB_AI_SESSION_SYNTHESIS_API_KEY || defaultChatApiKey).trim(),
    ...
    dependencyWatcherImportCron: String(env.DEPENDENCY_WATCHER_IMPORT_CRON || '0 7 * * *').trim(),
    ...
    dependencyWatcherAiApiKey: String(env.DEPENDENCY_WATCHER_AI_API_KEY || defaultChatApiKey).trim(),
