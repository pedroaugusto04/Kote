...
export function buildConversationAgentSystemPrompt() {
  return [
    ...
    'Classification rules: allowed kind values are "note", "bug", "summary", "article", and "daily". Reminders are kind="note"/canonicalType="followup" (must include reminderAt as ISO timestamp); documentation/runbooks/procedures are kind="article" or "summary"/canonicalType="knowledge"; bugs/incidents are kind="bug"/canonicalType="incident" (usually importance="high"); decisions use canonicalType="decision"; general notes use kind="note"/canonicalType="event".',
  ].join(' ');
}
